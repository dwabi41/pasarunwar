...
<?= $this->section('content') ?>
    <div class="container">
        <h3><span class="glyphicon glyphicon-file"></span>Pengumuman</h3>
        <ol class="breadcrumb">
            <li class="active">Data</li>
            <li><a href="<?php echo base_url('manage_pengumuman/tambah')?>">Add</a></li>
        </ol>
        <div class="row">
            ...
            <div class="col-md-6 col-xs-12">
                <?php echo form_open_multipart('manage_pengumuman/search','id = frmmenus'); ?>
                    <?= csrf_field() ?>
        ...
        <table class="table table-striped">
            ...
            if (!empty($datas))
            {
                ...
                foreach ($datas as $data){ ?>
                    <tr>
                        <td><?= $index ?></td>
                        <td><a href="<?= base_url('manage_pengumuman/edit/'.$data->id) ?>" class="btn btn-primary">Edit</a>&nbsp;&nbsp;&nbsp;<button onclick="confirmDelete(<?= $data->id?>,'<?= base_url('manage_pengumuman/delete')?>/')" class="btn btn-danger">Delete</button></td>
                        <td><?= $data->date ?></td>
                        <td><?= $data->title ?></td>
                        <td><?= $data->hit ?></td>
                        <?php 
