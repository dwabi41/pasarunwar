...
<?= $this->section('content') ?>
    <div class="container">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="<?php echo base_url('manage_kontak/tambah')?>">Add</a></li>
        </ol>
        <div class="row">
            ...
            <div class="col-md-6 col-xs-12">
                <?php echo form_open_multipart('manage_kontak/search','id = frmmenus'); ?>
                    <?= csrf_field() ?>
        ...
        <table class="table table-striped">
            ...
            if (!empty($datas))
            {
                ...
                foreach ($datas as $data){ ?>
                    <tr>
                        <td><?= $index ?></td>
                        <td><a href="<?= base_url('manage_kontak/edit/'.$data->id) ?>" class="btn btn-primary">Edit</a>&nbsp;&nbsp;&nbsp;<button onclick="confirmDelete(<?= $data->id?>,'<?= base_url('manage_kontak/delete')?>/')" class="btn btn-danger">Delete</button></td>
                        <td><?= $data->email ?></td>
                        <td><?= $data->name ?></td>
                        <?php 
                            if($data->status=='Show'){ ?>
